...
export interface MotorCalcInput {
  ...
  ccCategory: 'sub1000' | '1000-1500' | 'above1500' | 'bike'; // Engine CC category for TP rates
}
...
export function calculateMotorPremium(input: MotorCalcInput): PremiumBreakdown {
  ...
  // TP Premium — IRDAI 2025-26 Motor Tariff Rates
  // Source: IRDAI Motor Third-Party Premium Rates FY25
  const TP_RATES: Record<string, number> = {
    sub1000: 2094,      // ₹2,094 for sub-1000cc cars
    '1000-1500': 3316,  // ₹3,316 for 1000-1500cc cars
    above1500: 7789,    // ₹7,789 for 1500cc+ cars
    bike: 1366,         // ₹1,366 for two-wheelers
  };
  const ccKey = input.ccCategory || (input.vehicleType === 'bike' ? 'bike' : 'sub1000');
  const tpPremium = TP_RATES[ccKey] ?? 2094;
  loadings.push({ name: 'Third-party premium (IRDAI FY25)', amount: tpPremium });
