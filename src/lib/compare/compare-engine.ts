...
export function calculateMotorQuote(
...
): QuoteBreakdown & { addonBreakdown?: AddonBreakdownItem[] } {
  const notes: string[] = [];
  ...
  const depreciationRate = IDV_DEPRECIATION_RATES[depreciationKey] ?? 0.45;
  ...
  breakdown['IDV'] = idv;
  notes.push(`IDV: ${formatINR(idv)} (${(depreciationRate * 100).toFixed(0)}% depreciation, ${((1 - depreciationRate) * 100).toFixed(0)}% of ex-showroom)`);
  ...
  // --- 5. Add-On Premiums (using per-insurer rates) ---
  let addOnTotal = 0;
  const insurerAddonRates = ADDON_RATE_BY_INSURER[insurer];
  const addonDefMap = new Map(ADDON_DEFINITIONS.map((d) => [d.id, d]));

  for (const addon of selectedAddOns) {
    let addonPremium = 0;
    const addonDef = addonDefMap.get(addon);

    // Try per-insurer rate first
    if (insurerAddonRates && insurerAddonRates[addon]) {
      const rateInfo = insurerAddonRates[addon];
      if (rateInfo.type === 'percent') {
        // Percentage of OD premium before NCB
        addonPremium = Math.round((breakdown['OD_Premium_Before_NCB'] as number) * rateInfo.value);
      } else {
        // Flat rate
        addonPremium = rateInfo.value;
      }
    } else if (addonDef) {
      // Fallback to definition defaults
      if (addonDef.type === 'percent' && addonDef.defaultRatePercent !== undefined) {
        addonPremium = Math.round((breakdown['OD_Premium_Before_NCB'] as number) * addonDef.defaultRatePercent);
      } else if (addonDef.type === 'flat' && addonDef.defaultFlatRate !== undefined) {
        addonPremium = addonDef.defaultFlatRate;
      }
    } else {
      // Legacy fallback from ADDON_RATE
      const legacyRate = ADDON_RATE[addon as keyof typeof ADDON_RATE];
      if (legacyRate !== undefined) {
        const percentageAddOns: string[] = ['zeroDep', 'returnToInvoice', 'ncbProtect'];
        if (percentageAddOns.includes(addon)) {
          addonPremium = Math.round((breakdown['OD_Premium_Before_NCB'] as number) * legacyRate);
        } else {
          addonPremium = legacyRate as number;
        }
      }
    }

    if (addonPremium > 0 || addon === 'roadSideAssistance') {
      breakdown[`Addon_${addon}`] = addonPremium;
      addOnTotal += addonPremium;
      const label = addonDef?.label ?? addon;
      addonBreakdown.push({ id: addon, label, premium: addonPremium });
      notes.push(`Add-on ${label}: ${formatINR(addonPremium)}`);
    }
  }

  breakdown['AddOn_Total'] = addOnTotal;

  // --- 6. PA Cover (compulsory owner-driver) ---
