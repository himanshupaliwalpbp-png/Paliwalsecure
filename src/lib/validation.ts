...
// ── AI Claim Assessment validation ───────────────────────────────────────────
export const aiClaimAssessmentSchema = z.object({
  claimType: z.enum(['health', 'life', 'motor', 'travel', 'home']),
  claimAmount: z.number().positive('Claim amount must be positive'),
  sumInsured: z.number().positive('Sum insured must be positive'),
  policyAge: z.string().min(1, 'Policy age is required'),
  hasPED: z.boolean().optional().default(false),
  waitingPeriodCompleted: z.boolean().optional().default(false),
  hospitalType: z.enum(['network', 'non-network', 'any']).optional(),
  description: z.string().min(10, 'Please describe your claim in detail').max(5000),
  insurerCsr: z.number().min(0).max(100).optional(),
});
