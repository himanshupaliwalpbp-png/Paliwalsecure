...
import { Lightbulb, RefreshCw, ArrowLeft, Calendar } from 'lucide-react'
...
export function DailyTip() {
  ...
  // Track current day for auto-rotation
  const currentDayRef = React.useRef(getDayOfYear())
  ...
  // Auto-update daily: check every 60 seconds if the day has changed
  React.useEffect(() => {
    const interval = setInterval(() => {
      const newDay = getDayOfYear()
      if (newDay !== currentDayRef.current) {
        currentDayRef.current = newDay
        const newDayIndex = newDay % insuranceTips.length
        setCurrentIndex(newDayIndex)
      }
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-1">
                <Button
                  ...
                  className="h-8 gap-1 text-xs text-muted-foreground hover:text-blue-600 dark:hover:text-blue-400"
                  aria-label={t('dailyTip.prevTip')}
                >
            ...
            <div className="flex items-center justify-between text-xs text-muted-foreground">
              <span className="opacity-70">
                {t('dailyTip.tipOf')} {currentIndex + 1} {t('dailyTip.of')} {insuranceTips.length}
              </span>
              <div className="flex items-center gap-1.5 opacity-60">
                <Calendar className="w-3 h-3" />
                <span>{t('dailyTip.dayLabel')} {dayOfYear} {t('dailyTip.of')} {totalDays}</span>
              </div>
            </div>
          </motion.div>
        </AnimatePresence>
