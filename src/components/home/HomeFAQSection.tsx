  ...
  return (
    <section className="section-luxury section-luxury-alt">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <h2 className="text-4xl md:text-5xl font-extrabold text-[#043C50] dark:text-[#EED9BE] mb-4 font-heading tracking-tight">
            {heading} <span className="gradient-text-blue-emerald">{headingAccent}</span>
          </h2>
          <p className="text-lg text-[#D7C2A5] dark:text-[#D7C2A5] font-sans max-w-2xl mx-auto leading-relaxed">
            {subtitle}
          </p>
        </motion.div>
          ...
          <Accordion type="single" collapsible className="space-y-3">
            {faqs.map((faq, index) => (
              <AccordionItem
                key={index}
                value={`item-${index}`}
                className="premium-card border border-[#D7C2A5]/60 dark:border-white/[0.06] rounded-xl px-6 !p-0 data-[state=open]:border-[#043C50]/20 dark:data-[state=open]:border-[#3DB8D8]/30 data-[state=open]:shadow-premium transition-all duration-300 group"
              >
                <AccordionTrigger className="text-left font-heading text-[0.9375rem] font-semibold text-[#043C50] dark:text-[#EED9BE] hover:no-underline py-5 [&>svg]:text-[#D7C2A5] dark:[&>svg]:text-[#D7C2A5] [&>svg]:h-4 [&>svg]:w-4 [&>svg]:shrink-0 [&>svg]:transition-transform [&>svg]:duration-200">
                  {isHindi ? faq.question.hi : isEnglish ? faq.question.en : faq.question.hg}
                </AccordionTrigger>
                <AccordionContent className="text-[0.875rem] text-[#D7C2A5] dark:text-[#D7C2A5] leading-[1.7] font-sans pb-5">
        ...
        >
          <p className="text-[#D7C2A5] dark:text-[#D7C2A5] mb-5 font-sans text-sm">{stillQuestions}</p>
          <a
