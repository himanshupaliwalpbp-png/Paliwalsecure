  ...
  return (
    <div ref={containerRef} className="relative w-full max-w-md mx-auto">
      ...
      <div className="hidden lg:block" style={{ perspective: '1000px' }}>
        ...
        >
          {/* Main Shield Card */}
          <div className="relative rounded-3xl p-8 bg-white/80 dark:bg-white/10 backdrop-blur-xl border border-slate-200/60 dark:border-white/20 shadow-2xl dark:shadow-none">
            ...
            />
            <div className="absolute inset-[1px] rounded-3xl bg-gradient-to-br from-white to-slate-50 dark:from-slate-900/95 dark:to-blue-900/95" />
            ...
            <div className="relative z-10">
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h3 className="font-bold text-foreground dark:text-white text-lg">Coverage Score</h3>
              ...
              <div className="space-y-5">
                {[
                  { label: 'Health', pct: 85, gradient: 'from-rose-400 to-pink-600', color: 'text-rose-500 dark:text-rose-400' },
                  { label: 'Life', pct: 60, gradient: 'from-blue-400 to-indigo-600', color: 'text-blue-500 dark:text-blue-400' },
                  { label: 'Motor', pct: 92, gradient: 'from-amber-400 to-orange-600', color: 'text-amber-500 dark:text-amber-400' },
                ].map((bar) => (
                  <div key={bar.label}>
                    <div className="flex justify-between text-sm mb-1.5">
                      <span className="text-foreground/70 dark:text-slate-300">{bar.label}</span>
              ...
              <div className="mt-6 p-3 bg-amber-500/10 dark:bg-amber-500/10 rounded-xl border border-amber-500/20">
      ...
      <div className="lg:hidden" style={{ perspective: '800px' }}>
        <motion.div
          animate={{
            scale: [1, 1.03, 1],
          }}
        ...
        >
          <div className="glass dark:glass-dark rounded-3xl p-6 shadow-2xl">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="font-bold text-foreground dark:text-white">Coverage Score</h3>
            ...
            <div className="mt-4 p-2.5 bg-amber-500/10 rounded-xl border border-amber-500/20">
              <p className="text-xs text-amber-600 dark:text-amber-300 font-medium">2 gaps found in your coverage</p>
            </div>
          </div>
        </motion.div>
      </div>
      ...
      {/* Floating Badges */}
      <motion.div
        className="hidden lg:block absolute -top-4 -right-4 glass dark:glass-dark rounded-xl p-3 shadow-lg z-20"
        animate={{ y: [-5, 5, -5] }}
        transition={{ duration: 3, repeat: Infinity, ease: 'easeInOut' as const }}
      >
        <div className="flex items-center gap-2">
          <Brain className="w-5 h-5 text-indigo-500 dark:text-indigo-400" />
          <span className="text-sm font-semibold text-foreground dark:text-white">AI Ready</span>
        </div>
      </motion.div>

      <motion.div
        className="hidden lg:block absolute -bottom-4 -left-4 glass dark:glass-dark rounded-xl p-3 shadow-lg z-20"
        animate={{ y: [5, -5, 5] }}
        transition={{ duration: 4, repeat: Infinity, ease: 'easeInOut' as const }}
      >
        <div className="flex items-center gap-2">
          <ShieldCheck className="w-5 h-5 text-blue-500 dark:text-blue-400" />
          <span className="text-sm font-semibold text-foreground dark:text-white">IRDAI Verified</span>
        </div>
      </motion.div>
