...
function BreakdownCard({ breakdown, type }: { breakdown: PremiumBreakdown; type: 'health' | 'term' | 'motor' }) {
  ...
  return (
    ...
    >
      <Card className="overflow-hidden border-0 shadow-xl rounded-2xl bg-card max-w-full">
        {/* Header gradient */}
...
function HealthTab() {
  ...
  return (
    <div className="grid lg:grid-cols-5 gap-6 w-full max-w-full overflow-x-hidden">
      {/* Form */}
      ...
      {/* Result + 3D Character */}
      <div className="lg:col-span-2 min-w-0 max-w-full space-y-4">
        {/* 3D Character */}
        <div className="flex justify-center">
          <FormCharacterWithAutoMood
            formStage={age > 50 ? 3 : age > 30 ? 2 : 1}
            hasResult={!!result}
        ...
        <AnimatePresence mode="wait">
          {result ? (
            ...
            <motion.div
              ...
              exit={{ opacity: 0 }}
            >
              <Card className="glass-card rounded-2xl border-0 shadow-lg bg-card max-w-full overflow-hidden">
...
function MotorTab() {
  ...
  const [ccCategory, setCcCategory] = useState<'sub1000' | '1000-1500' | 'above1500'>('sub1000');
  const [result, setResult] = useState<PremiumBreakdown | null>(null);
  const [isCalculating, setIsCalculating] = useState(false);

  // ── IRDAI 2025-26 Motor TP Rates ── Source: IRDAI Motor Tariff FY25
  const MOTOR_TP_RATES: Record<string, number> = {
    sub1000: 2094,    // ₹2,094 for sub-1000cc
    '1000-1500': 3316, // ₹3,316 for 1000-1500cc
    above1500: 7789,  // ₹7,789 for 1500cc+
    bike: 1366,       // ₹1,366 for two-wheelers
  };
  ...
  const calculate = useCallback(() => {
    ...
    setTimeout(() => {
      const input: MotorCalcInput = {
        ...
        ncb: Number(ncb),
        addOns,
        ccCategory: vehicleType === 'bike' ? 'bike' : ccCategory,
      };
      setResult(calculateMotorPremium(input));
      setIsCalculating(false);
    }, 800);
  }, [vehicleType, regYear, idv, ncb, addOns, ccCategory]);
      ...
      <div className="lg:col-span-3 space-y-6 min-w-0">
        <Card className="glass-card rounded-2xl border-0 shadow-lg bg-card">
          <CardContent className="p-6 space-y-6">
            ...
            {/* CC Category (Engine Capacity) — IRDAI 2025-26 TP Rates */}
            {vehicleType === 'car' && (
              <div className="space-y-2">
                <Label className="text-sm font-semibold flex items-center">
                  Engine Capacity (CC)
                  <InfoTip text="Third-party premium CC (cubic capacity) pe depend karta hai. Bada engine = zyada TP premium." />
                </Label>
                <div className="grid grid-cols-3 gap-2">
                  {[
                    { value: 'sub1000' as const, label: '< 1000cc', tp: '₹2,094' },
                    { value: '1000-1500' as const, label: '1000-1500cc', tp: '₹3,316' },
                    { value: 'above1500' as const, label: '> 1500cc', tp: '₹7,789' },
                  ].map((opt) => (
                    <button
                      key={opt.value}
                      onClick={() => setCcCategory(opt.value)}
                      className={`px-3 py-2.5 rounded-xl text-center transition-all duration-200 border ${
                        ccCategory === opt.value
                          ? 'bg-amber-50 border-amber-300 text-amber-700 dark:bg-amber-950/40 dark:border-amber-700 dark:text-amber-300 shadow-sm'
                          : 'bg-background border-border text-muted-foreground hover:border-amber-200'
                      }`}
                    >
                      <p className="text-sm font-semibold">{opt.label}</p>
                      <p className="text-[10px] opacity-70">TP: {opt.tp}/yr</p>
                    </button>
                  ))}
                </div>
              </div>
            ...
            <div className="flex items-start gap-2 p-3 rounded-xl bg-amber-50 dark:bg-amber-950/30 border border-amber-200 dark:border-amber-800">
              ...
              <p className="text-xs text-amber-700 dark:text-amber-300 leading-relaxed">
                <span className="font-semibold">TP rates may increase 18-25% soon.</span> IRDAI reviews TP rates periodically. Current rates are as per FY25 tariff.
              </p>
            </div>
