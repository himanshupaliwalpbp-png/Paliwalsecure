...
const AnimatedBadge = ({
  ...
  const content = (
    <motion.div
      ...
      viewport={{ once: true }}
      className="group relative flex max-w-fit items-center justify-center gap-3 rounded-full border border-neutral-300 bg-white px-4 py-1.5 text-neutral-700 transition-colors dark:border-[#C98A1C]/30 dark:bg-[#0A1330] dark:text-zinc-300"
    >
      <div className="pointer-events-none absolute inset-x-0 bottom-full h-20 w-[165px]">
        <svg
          ...
          fill="none"
        >
          <g mask="url(#ml-mask-1-ps)">
            <circle
              className="multiline ml-light-1"
              cx="0"
              cy="0"
              r="20"
              fill="url(#ml-white-grad-ps)"
            />
          </g>
          <defs>
            <mask id="ml-mask-1-ps">
              <path
                d="M 69 49.8 h -30 q -3 0 -3 -3 v -13 q 0 -3 -3 -3 h -23 q -3 0 -3 -3 v -13 q -3 0 -3 -3 h -30"
                strokeWidth="0.6"
                stroke="white"
              />
            </mask>
            <radialGradient id="ml-white-grad-ps" fx="1">
      ...
      <div className="mx-2 h-4 w-px bg-neutral-300 dark:bg-[#C98A1C]/25" />
      <span className="bg-clip-text text-xs font-medium">{text}</span>
      {href && (
        <ChevronRight className="ml-1 h-3.5 w-3.5 text-neutral-400 transition-transform duration-200 group-hover:translate-x-0.5 dark:text-[#C98A1C]/50" />
      )}
    </motion.div>
  );
...
.ml-light-1 {
  offset-path: path(
    "M 69 49.8 h -30 q -3 0 -3 -3 v -13 q 0 -3 -3 -3 h -23 q -3 0 -3 -3 v -13 q -3 0 -3 -3 h -50"
  );
}
