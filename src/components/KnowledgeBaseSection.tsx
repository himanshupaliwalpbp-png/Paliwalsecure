...
type TrendCategory = 'all' | 'premium-hike' | 'market-growth' | 'tech-shift' | 'regulatory' | 'consumer-behavior';
type RegulationCategory = 'all' | 'ped' | 'moratorium' | 'claims' | 'gst' | 'consumer-protection' | 'portability';
...
export default function KnowledgeBaseSection() {
  ...
  const tabConfig = [
    { value: 'glossary', label: t('knowledge.cat.glossary'), icon: BookOpen, count: policyGlossary.length },
    { value: 'articles', label: t('knowledge.cat.articles'), icon: Sparkles, count: blogArticles.length },
    { value: 'myths', label: t('knowledge.cat.mythBusters'), icon: AlertTriangle, count: mythBusters.length },
    { value: 'compare', label: t('knowledge.cat.compare'), icon: TrendingUp, count: insuranceCompanies.length },
    { value: 'trends', label: t('knowledge.cat.marketTrends'), icon: TrendingUp, count: marketTrends2026.length },
    { value: 'regulations', label: t('knowledge.cat.irdaiRules'), icon: Shield, count: irdaiRegulations2025.length },
    { value: 'claims', label: t('knowledge.cat.claimGuide'), icon: Clock, count: claimGuides.length },
  ];
          ...
          <div className="max-h-[600px] overflow-y-auto pr-1 custom-scrollbar">
                    ...
                    <AccordionContent className="text-sm text-muted-foreground pb-4 space-y-3">
                      ...
                      {term.example && (
                        <div className="p-3 bg-muted/50 rounded-xl border border-border/30">
                          <p className="text-xs font-medium text-foreground mb-1">{t('knowledge.example')}</p>
            ...
            {filteredGlossary.length === 0 && (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-sm">{t('knowledge.noTermsFound')}</p>
              </div>
            )}
          </div>
        </TabsContent>
        ...
        <TabsContent value="articles" className="space-y-4">
          ...
          {filteredArticles.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">{t('knowledge.noArticles')}</p>
            </div>
          )}
        </TabsContent>
        ...
        <TabsContent value="myths" className="space-y-4">
            ...
            {filteredMyths.map((myth, i) => (
              ...
              >
                ...
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div>
                    <p className="text-[10px] font-semibold text-rose-600 dark:text-rose-400 uppercase tracking-wider mb-1">{t('knowledge.mythLabel')}</p>
                ...
                <div className="flex items-start gap-3">
                  ...
                  <div>
                    <p className="text-[10px] font-semibold text-green-600 dark:text-green-400 uppercase tracking-wider mb-1">{t('knowledge.factLabel')}</p>
          ...
          {filteredMyths.length === 0 && (
            <div className="text-center py-12 text-muted-foreground">
              ...
              <p className="text-sm">{t('knowledge.noMyths')}</p>
            </div>
          )}
        </TabsContent>
          ...
          <div className="flex flex-col sm:flex-row gap-3 sm:items-center justify-between">
            <div className="flex gap-2 flex-wrap">
              {(['all', 'health', 'life'] as CompareCategory[]).map(cat => (
                ...
                >
                  {cat === 'all' ? t('knowledge.allCompanies') : cat === 'health' ? t('knowledge.healthGeneral') : t('knowledge.lifeInsurance')}
                </button>
              ))}
            </div>
