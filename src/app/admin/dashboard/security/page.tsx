...
export default function SecurityPage() {
  ...
  const [envInstructions, setEnvInstructions] = useState<{
    name: string;
    value: string;
    steps: string[];
  } | null>(null);
  ...
  const [disableEnvInstructions, setDisableEnvInstructions] = useState<string[] | null>(null);
  ...
  const handleVerify = async () => {
    ...
    try {
      const res = await fetch('/api/admin/auth/mfa/verify', {
        ...
        body: JSON.stringify({ token: verifyCode, setupToken }),
      });
      const data = await res.json();
      if (!data.success) throw new Error(data.error || 'Verification failed');

      if (data.isEnvSetup && data.envVarInstruction) {
        // On Vercel, show instructions to add env var
        setEnvInstructions(data.envVarInstruction);
        setSecretKey(data.totpSecret || secretKey);
        setSetupStep('setup'); // Show env var setup instructions
      } else {
        setMfaEnabled(true);
        setSetupStep('idle');
        setVerifyCode('');
  ...
  const handleDisable = async () => {
    ...
    try {
      ...
      if (data.isEnvSetup && data.envVarInstruction) {
        // On Vercel, show instructions to remove env var
        setDisableEnvInstructions(data.envVarInstruction.steps);
      } else {
        setMfaEnabled(false);
  ...
  // ── Copy env var value to clipboard ────────────────────────────────────
  const copyEnvValue = () => {
    navigator.clipboard.writeText(secretKey);
  ...
  return (
    <div className="space-y-6 max-w-2xl">
      ...
      {/* ── Vercel Env Setup Instructions (shown after verify on Vercel) ──── */}
      {envInstructions && (
        <Card className="border-0 shadow-md border-l-4 border-l-emerald-500">
          <CardHeader>
            <div className="flex items-center gap-2">
              <CheckCircle2 className="w-5 h-5 text-emerald-500" />
              <CardTitle className="text-base">MFA Verification Successful!</CardTitle>
            </div>
            <CardDescription>
              To complete MFA setup on Vercel, you need to add the secret as an environment variable.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="p-4 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-sm text-blue-700 font-medium mb-2">Follow these steps:</p>
              <ol className="space-y-1">
                {envInstructions.steps.map((step, i) => (
                  <li key={i} className="text-sm text-blue-600">{step}</li>
                ))}
              </ol>
            </div>
            <div>
              <Label className="text-sm font-medium">Environment Variable Name</Label>
              <code className="block bg-slate-100 px-3 py-2 rounded-lg text-sm font-mono mt-1">
                {envInstructions.name}
              </code>
            </div>
            <div>
              <Label className="text-sm font-medium">Environment Variable Value</Label>
              <div className="flex items-center gap-2 mt-1">
                <code className="flex-1 bg-slate-100 px-3 py-2 rounded-lg text-xs font-mono break-all select-all">
                  {secretKey}
                </code>
                <Button variant="outline" size="icon" onClick={copyEnvValue} className="shrink-0">
            ...
            <Button
              ...
              onClick={() => {
                ...
                setSecretKey('');
                setSetupToken(null);
                fetchMfaStatus(); // Re-fetch status
              }}
            >
      ...
      {/* ── Vercel Disable Instructions (shown after disable on Vercel) ──── */}
      {disableEnvInstructions && (
        <Card className="border-0 shadow-md border-l-4 border-l-amber-500">
          <CardHeader>
            <div className="flex items-center gap-2">
              <AlertTriangle className="w-5 h-5 text-amber-500" />
              <CardTitle className="text-base">Complete MFA Disable on Vercel</CardTitle>
            </div>
            <CardDescription>
              MFA verification passed. Remove the environment variable to complete disabling.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="p-4 bg-amber-50 border border-amber-200 rounded-lg">
              <ol className="space-y-1">
                {disableEnvInstructions.map((step, i) => (
            ...
            <Button
              variant="outline"
              onClick={() => {
                setDisableEnvInstructions(null);
                setShowDisable(false);
                setDisablePassword('');
                setDisableCode('');
                fetchMfaStatus();
            ...
            >
              I&apos;ve Removed the Environment Variable
            </Button>
          </CardContent>
        </Card>
      )}
        ...
        <CardContent className="space-y-4">
          {mfaFetching ? (
              ...
              {isEnvSetup && (
                <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <p className="text-xs text-blue-600">
                    <strong>Note:</strong> MFA is configured via environment variables on this server.
                    Changes require updating environment variables in your hosting provider settings.
                  </p>
                </div>
              )}
            ...
            <div className="space-y-4">
              ...
              {setupStep === 'setup' && !envInstructions && (
                /* This state is used when we need to show additional setup info */
                <div className="space-y-4">
